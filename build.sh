...
for file in libbindy.so libximc.so.${major} libxibridge.so ; do cp $wd/usr/lib/$file ./$r_dir/usr/lib/ ; done
...
# add libximc framework (with xibridge and bindy inside) to the bundle
