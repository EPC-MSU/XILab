...
cd "%DISTDIR%"
7z a %BASEDIR%\mDDC-win32.7z win32\
@if not %errorlevel% == 0 goto FAIL
7z a %BASEDIR%\mDDC-win64.7z win64\
@if not %errorlevel% == 0 goto FAIL
...
@if not %errorlevel% == 0 goto FAIL
move /Y %ARCH%\%BINDIR%\"mDDC "[%BINDIR%].exe a.exe
@if not %errorlevel% == 0 goto FAIL
...
@if not %errorlevel% == 0 goto FAIL
move /Y a.exe %ARCH%\%BINDIR%\"mDDC "[%BINDIR%].exe
@if not %errorlevel% == 0 goto FAIL
copy %ARCH%\%BINDIR%\"mDDC ["%BINDIR%"].exe" %DISTARCH%
@if not %errorlevel% == 0 goto FAIL
...
@if not %errorlevel% == 0 goto FAIL
ren mDDC-*.exe mDDC-*.exe
@if not %errorlevel% == 0 goto FAIL
:: Sign binaries only in release build
@if 'x%XIMC_RELEASE_TYPE%' == 'xRELEASE' SignTool.exe sign /v /n %CERTNAME% mDDC-*.exe
@if not %errorlevel% == 0 goto FAIL
@goto :eof
