    ...
    : QMainWindow(parent), ui(new Ui::MainWindowClass)
{
	...
	QObject::connect(ui->TechSupport, SIGNAL(clicked()), this, SLOT(OnTechSupportClicked()));
...
void MainWindow::InitUI()
{
	...
	homdLbl.setToolTip(tr("Lights up after successful execution of home() command meaning that relative position \nscale is calibrated against a hardware absolute position sensor like a limit switch. \nDrops after loss of calibration like harsh stop and possibly skipped steps."));
	...
	pwhtLbl.setToolTip(tr("Power driver overheat. Motor control is disabled until some cooldown. \nThis should not happen in boxed versions of controller. \nThis may happen in bare board version of controller with a custom radiator. \nRedesign your radiator then."));
	...
	engLbl.setToolTip(tr("Lights up red when the motor control error occurs. \nMotor control algorithm failure means that it can't \ndefine the correct decisions with the feedback data it receives. \nSingle failure may be caused by mechanical problem. \nA repeating failure can be caused by incorrect motor settings."));
	engLbl.setFont(font);
	...
	errdLbl.setToolTip(tr("Data integrity error encountered. \nThe data inside command and its CRC code do not correspond, \ntherefore data can't be considered valid. \nThis error may be caused by EMI in UART/RS232 interface."));
	...
	errvLbl.setToolTip(tr("Value error encountered. The values in the command can't be applied \nwithout correction because they fall out the valid range. \nCorrected values were used instead of the orginal ones."));
...
void MainWindow::UpdateLogTable()
{
	...
	QString message_old = "";
	int counter_mess = 1;
	int row1 = ui->logEdit->rowCount();

	if (row1 > 0) {
		row1--;
		message_old = ui->logEdit->item(row1, 3)->text();
	}		
...
QString getServiceMailInfo(const QString line_sep)
{
	...
	result = devinterface->get_firmware_version(&inf1.Major, &inf1.Minor, &inf1.Release);
	service_info.append(QString::number(inf1.Major) + "." + QString::number(inf1.Minor) + "." + QString::number(inf1.Release));
	...
	result = devinterface->get_serial_number(&data2);
	service_info.append(QString::number(data2));
...

	{
		...
		if (result == result_ok)
		{
			...
			service_info.append(stage_inf.PartNumber);// stage_name1.PositionerName
		}
	}
	else
	{
...
void MainWindow::OnTechSupportClicked()
{
...
#endif		
	{
		...
		mail_data.append(getServiceMailInfo(sep));
