	...
	*/
	...
	if (dss->Enumerate_network)
	{
		if (dss->url.size() == 0)
		{
			...
			*/
			qs = QString("addr=");
			for (int i = 0; i < dss->url.size(); i++) {
				if (QString::compare(dss->url.at(i).first, QString("xi-udp://"), Qt::CaseInsensitive) == 0)
					qs.append(QString("xi-udp://")).append(dss->url.at(i).second).append(",");
				else
					if (QString::compare(dss->url.at(i).first, QString("xi-tcp://"), Qt::CaseInsensitive) == 0)
						qs.append(QString("xi-tcp://")).append(dss->url.at(i).second).append(",");
					else
						qs.append(dss->url.at(i).second).append(",");
